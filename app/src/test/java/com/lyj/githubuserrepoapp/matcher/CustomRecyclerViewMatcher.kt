...
object CustomRecyclerViewMatcher {
    ...
    fun withHolderAssertion(position : Int, assertion: (RecyclerView.ViewHolder) -> Boolean): Matcher<View> =
        object : BoundedMatcher<View, RecyclerView>(RecyclerView::class.java) {
            override fun describeTo(description: Description) {
                description.appendText("recyclerview item count test")
            }
            ...
            override fun matchesSafely(view: RecyclerView): Boolean {
                return assertion(view.findViewHolderForAdapterPosition(position)!!)
            }
        }
